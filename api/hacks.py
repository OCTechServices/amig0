...
class handler(BaseHTTPRequestHandler):
    ...
    def _discover_prompt(self, city, preferences, session=1):
        ...
        return (
            ...
            f"{angle}\n\n"
            f"HARD RULE — Category diversity: The 5 venues must span different categories. No two venues "
            f"may share the same category (e.g. no two bars, no two cafés, no two restaurants). Each venue "
            f"must bring something categorically distinct to the list.\n\n"
            f"Step 1: Identify exactly 5 venues, bars, galleries, trails, or experiences in {city} that pass all "
    ...
    def _spotlight_prompt(self, city, venue, site_content=''):
        ...
        return (
            ...
            f"The tone is confident, specific, and warm — like a friend who works there tipping you off.\n\n"
            ...
            "- Tip quality: every tip must contain a concrete action — what to ORDER, ASK FOR, TIME, or AVOID. "
            "Maximum 2 sentences. Never open with a description of the venue or its atmosphere.\n"
            "  Bad: 'This bar is known for its craft beer selection.'\n"
            "  Good: 'Ask the bartender for the small-batch seasonal tap — it\\'s not on the board but they always have one.'\n"
            "- website: official website URL if you are confident it is correct — omit or leave empty string if unsure.\n"
            "- instagram: Instagram handle (without @) if you are confident it is correct — omit or leave empty string if unsure.\n"
