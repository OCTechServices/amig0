...
| I27 | Content Engine authorization gap — PUBLISH_SECRET transmitted by client-side JS | Session 21 / S01 | Closed — Phase 1 implementation complete. PUBLISH_SECRET removed. api/ig-post.py now requires Firebase ID token (Authorization: Bearer) verified server-side with firebase-admin. content/index.html: operator sign-in gate added, PUBLISH_TOKEN constant removed, _getAuthHeader() injects ID token. Pending production deployment (RED gate). |
