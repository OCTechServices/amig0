...
| I28 | Deals subscription gate is client-side only — server-side enforcement gap | Phase 0 Reconciliation / S02 | Closed — Evidence-based assessment. Firestore rules already enforce server-side authorization for all redemptions: (1) isMemberAllowed() verifies subscriptionStatus server-side, (2) PIN verified server-side via get(affiliates/secrets/verifyPin), (3) isNotRecentlyRedeemed() 30-day cooldown server-side. deals.html client-side isAccessAllowed() is documented as "UX gate only; this rule is the real lock." No Firestore rule change required. |
