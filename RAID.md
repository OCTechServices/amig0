...
| I27 | Content Engine authorization gap — PUBLISH_SECRET transmitted by client-side JS | Session 21 / S01 | Closed — COMPLETE (2026-09-14). Evidence: (1) CLAIM ASSIGNED — operator:true set via Firebase Admin SDK. (2) SERVER-SIDE CLAIM VERIFIED — read-back confirmed immediately after assignment. (3) FRESH-TOKEN CLAIM OBSERVED — JWT payload decoded from getIdToken(true) confirmed operator:true. (4) OPERATOR REQUEST ACCEPTED — valid operator token traversed full auth path, reached VALIDATE stage (HTTP 400 "At least 2 slides required"); Firebase Admin init confirmed. (5) INVALID/FORGED TOKEN REJECTED — token with invalid signature → 403 AUTH "Invalid token". (6) VALID AUTHENTICATED NON-OPERATOR PATH — NOT TESTED (no second Firebase account available without claim modification; does not reopen S01). (7) HUMAN BROWSER VALIDATION — Daniel confirmed Content Engine accessible after sign-in, operator gate functional. (8) LEGACY PUBLISH_SECRET — removed from Vercel Production 2026-09-14. Application has no X-Publish-Token path. |
| I28 | Deals entitlement integrity gap — amig0_members field authority | Phase 0 Reconciliation / S02 | Open — PRODUCTION DEPLOYMENT AUTHORIZED 2026-09-15. Two gates complete. Security finding inventory (S02 namespace — distinct from Vision Backlog V-IDs): S02-V01/V02 HIGH REMEDIATED — isMemberAllowed() reads subscriptionStatus + trialEndsAt from user-writable doc; remediated: diff().affectedKeys().hasOnly(['zeroVariant']) locks update to zeroVariant only. S02-V03 HIGH REMEDIATED — sentinel deletable by client, bypasses 30-day lockout; remediated: delete rule absent. S02-V04 LOW OPEN/DEFERRED — client-side PIN brute-force throttling; no rate limit on deal_redemptions PIN attempts at rules layer; deferred, acceptable residual risk for current affiliate count. S02-V05 MEDIUM REMEDIATED — hostile client can write deal_redemptions without sentinel (omits sentinel, bypasses cross-month lockout); remediated: sentinelAdvanced() using existsAfter() + getAfter() enforces post-batch coupling invariant. S02-V06 LOW REMEDIATED — client can supply far-future redeemedAt on sentinel write, extending own lockout artificially; remediated: upper-bound timestamp rule (subtraction form: request.time >= redeemedAt - 5min). Implementations: api/init-trial.py (server-side trial init), deals.html ensureMemberDoc() refactored. 34/34 emulator tests (M01-M11, R01-R12, T01-T03, C01-C08). Commits: gate-1 c06c0e0, gate-2 61cf694. PRODUCTION DEPLOYED 2026-09-15. Vercel deployment dpl_BEprvPcMc4oBC1K4XuZnL9FmW2Rc (amig0.com). Firestore rules deployed to amig0-travel-company-52fb1 — compiled clean, released. Stage 1 validation: /api/init-trial → 401 unauthenticated ✓, 401 invalid token ✓, /deals 200 ✓. Stage 2 validation: affiliates public read 200 ✓, /deals HTML intact ✓, /hacks /business 200 ✓. Adversarial production checks NOT EXECUTED (no safe test identity available) — grounded in 34/34 emulator evidence. Redemption validation NOT EXECUTED (no designated test affiliate). No production errors observed at deployment. CLOSED. |
