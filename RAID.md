...
| I27 | Content Engine authorization gap — PUBLISH_SECRET transmitted by client-side JS | Session 21 / S01 | Closed — IMPLEMENTED (Session 21): PUBLISH_SECRET removed, Firebase ID token + operator claim enforced. DEPLOYED (2026-09-14): dpl_52x3T1t6W7rs1KLxwsVAezXiSYDW, amig0.com. TESTED in production: unauthenticated → 403 AUTH; forged token → 403 AUTH; valid operator token → passes AUTH, reaches VALIDATE; Firebase Admin init confirmed via successful token verification; PUBLISH_TOKEN not present in served HTML. Legacy PUBLISH_SECRET env var still in Vercel — removal is next RED cleanup action (not yet authorized). |
