...
| I28 | Deals entitlement integrity gap — amig0_members field authority | Phase 0 Reconciliation / S02 | Open — IMPLEMENTED LOCALLY, EMULATOR TESTED (2026-09-15). Prior "Closed" status was premature; deep S02 assessment found three material vulnerabilities: (V01/V02 HIGH) isMemberAllowed() reads subscriptionStatus and trialEndsAt from user-writable amig0_members doc — authenticated user could write subscriptionStatus:'active' to bypass subscription gate without paying; (V03 HIGH) redemption sentinel deletable by authenticated user — removal of sentinel bypasses isNotRecentlyRedeemed() 30-day lockout. Remediation implemented locally: (1) firestore.rules — amig0_members client update locked to zeroVariant only via diff() semantics; no client create; delete blocked. Sentinel — create/update only with fresh-timestamp + non-decreasing constraints; delete blocked. (2) api/init-trial.py — new serverless endpoint: Firebase ID token required, UID from verified token only, server chooses all trial timestamps, idempotent. (3) deals.html ensureMemberDoc() refactored: no longer creates amig0_members doc directly; calls POST /api/init-trial instead. (4) 26/26 emulator tests pass including explicit T01/T02/T03 threat path reproduction. Existing-user compatibility preserved: Admin SDK writes bypass rules; zeroVariant updates unaffected; Stripe webhook unaffected. No production data migration required. Stripe unchanged. PRODUCTION DEPLOYMENT NOT YET AUTHORIZED. Owner: Daniel to authorize firebase deploy --only firestore:rules + vercel --prod. |
