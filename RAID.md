...
# Last Updated: 2026-09-15 (S02 Final Security Gate — V05 remediation)
...
| I28 | Deals entitlement integrity gap — amig0_members field authority | Phase 0 Reconciliation / S02 | Open — IMPLEMENTED LOCALLY, EMULATOR TESTED (2026-09-15). Two gates complete. PRODUCTION DEPLOYMENT NOT YET AUTHORIZED. Gate 1 (commit c06c0e0): V01/V02 HIGH — isMemberAllowed() reads subscriptionStatus + trialEndsAt from user-writable doc — client could self-promote; remediated: diff().affectedKeys().hasOnly(['zeroVariant']) locks client update to cosmetic field only. V03 HIGH — sentinel deletable by client — deletion bypasses 30-day lockout; remediated: delete rule absent on sentinel subcollection. api/init-trial.py added: server-side trial init (Firebase ID token auth, UID from verified token, server timestamps). deals.html ensureMemberDoc() refactored to call POST /api/init-trial instead of writing Firestore directly. Gate 2 (this session): V05 MEDIUM — hostile client can write deal_redemptions without sentinel (omitting sentinel from batch bypasses 30-day cross-month lockout); remediated: sentinelAdvanced() helper using existsAfter() + getAfter() enforces coupling invariant in deal_redemptions create rule. Also fixed: (a) sentinel create/update upper-bound timestamp check — prevents far-future redeemedAt (C06 attack); (b) existsAfter() replaces exists(getAfter()) — latter unsupported in emulator/production for post-transaction existence checks; (c) timestamp-addition form rewritten as subtraction to avoid emulator eval error on serverTimestamp writes. 34/34 emulator tests pass (M01-M11, R01-R12, T01-T03, C01-C08). Existing-user compatibility preserved. Stripe unchanged. Owner: Daniel to authorize firebase deploy + vercel --prod. |
